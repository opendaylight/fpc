...
public class DpdkImpl implements Activator {
    ...
    public boolean canActivate() {
        if (dpnHolder.dpn == null) return false;
     ...
     * @param commands - 3GPP Instructions
        ...
        if  (opType == OpType.Create) {
            ...
            if (commands.getInstr3gppMob().isSession()) {
                ...
                if (context.getUl().getMobilityTunnelParameters().getMobprofileParameters() instanceof ThreeGPPTunnel) {
                    s1u_sgw_gtpu_teid = ((ThreeGPPTunnel) context.getUl().getMobilityTunnelParameters().getMobprofileParameters())
                            .getTunnelIdentifier();
                } else
                ...
                api.create_session(dpnTopic, threeProps.getImsi().getValue(),
                        IPToDecimal.cidrBase(assignedPrefix.getIpv4Prefix().getValue()),
                        threeProps.getEbi().getValue(),
                        context.getUl().getTunnelLocalAddress().getIpv4Address(),
                        s1u_sgw_gtpu_teid);
                txMessages.incrementAndGet();
                // TODO - Mod create_session to include TFT for X2 with SGW re-selection

                if (commands.getInstr3gppMob().isDownlink()) {
                    // SGW Re-selection
                    Long s1u_enb_gtpu_teid;
                    if (context.getUl().getMobilityTunnelParameters().getMobprofileParameters() instanceof ThreeGPPTunnel) {
                        s1u_enb_gtpu_teid = ((ThreeGPPTunnel) context.getUl().getMobilityTunnelParameters().getMobprofileParameters())
                                .getTunnelIdentifier();
                    } else
                        throw new Exception("Session Create with implied DL information but no DL Tunnel Info provided");
                    api.modify_bearer_dl(dpnTopic,
                            s1u_sgw_gtpu_teid,
                            context.getDl().getTunnelRemoteAddress().getIpv4Address(),
                            s1u_enb_gtpu_teid);
                    txMessages.incrementAndGet();
                }
                ...
                if (context.getDl().getMobilityTunnelParameters().getMobprofileParameters() instanceof ThreeGPPTunnel) {
                    dl_gtpu_teid = ((ThreeGPPTunnel) context.getDl().getMobilityTunnelParameters().getMobprofileParameters())
                            .getTunnelIdentifier();
                } else
                    throw new Exception("Session Create Requested but no UL Tunnel Info provided");
                ...
                api.create_bearer_ul(dpnTopic,
                        threeProps.getImsi().getValue(),
            ...
            //Update
            Long s1u_enb_gtpu_teid = ZERO, s1u_sgw_gtpu_teid  = ZERO;

            if (commands.getInstr3gppMob().isDownlink()) {
                if (context.getDl().getMobilityTunnelParameters().getMobprofileParameters() instanceof ThreeGPPTunnel) {
                    s1u_enb_gtpu_teid = ((ThreeGPPTunnel) context.getDl().getMobilityTunnelParameters().getMobprofileParameters())
                            .getTunnelIdentifier();
                } else
                    throw new Exception("Modify Bearer but no 3GPP DL Tunnel Info provided");
                boolean isDLdelete = (context.getDl().getLifetime() == null) ? false :
                    (context.getDl().getLifetime() == 0L);
                if (!isDLdelete) {
                    s1u_sgw_gtpu_teid = ((ThreeGPPTunnel) context.getUl().getMobilityTunnelParameters().getMobprofileParameters())
                            .getTunnelIdentifier();
                    api.modify_bearer_dl(dpnTopic,
            ...
            if (commands.getInstr3gppMob().isUplink())  {
                ...
                boolean isULdelete = (context.getUl().getLifetime() == null) ? false :
                    (context.getUl().getLifetime() == 0L);
                if (!isULdelete) {
                    s1u_enb_gtpu_teid = ((ThreeGPPTunnel) context.getDl().getMobilityTunnelParameters().getMobprofileParameters())
                            .getTunnelIdentifier();
                    api.modify_bearer_ul(dpnTopic,
                            context.getUl().getTunnelLocalAddress().getIpv4Address(),
                            s1u_enb_gtpu_teid,
                            s1u_sgw_gtpu_teid,
                            null);
            ...
            if (!commands.getInstr3gppMob().isSession()) {
                //LOG.info("Sending Message");
                api.delete_bearer(dpnTopic,
                        s1u_sgw_gtpu_teid);
                txMessages.incrementAndGet();
            } else {
                // Bearer Delete seems odd - how does DL get deleted
                //LOG.info("Sending Message");
                api.delete_session(dpnTopic,
                        threeProps.getLbi().getValue(),
                        s1u_sgw_gtpu_teid);
                txMessages.incrementAndGet();
            }
        }
    ...
    @Override
    public void delete(Instructions instructions, Targets target,
            FpcContext context) throws Exception {
        rxMessages.incrementAndGet();
        Long teid =  (context.getUl().getMobilityTunnelParameters().getMobprofileParameters() instanceof ThreeGPPTunnel) ?
            ((ThreeGPPTunnel) context.getUl().getMobilityTunnelParameters().getMobprofileParameters()).getTunnelIdentifier()
            : null;
    ...
    public Object read(Targets target) throws Exception {
        rxMessages.incrementAndGet();
        throw new UnsupportedOperationException("Use a generic Config Store Reader for this.  This protocol does not support reads.");
    }
